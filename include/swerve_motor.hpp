...
class swerve_motor {
   private:
	static constexpr auto MOTOR_ID = size_t{};
   ...
   public:
	swerve_motor();
	swerve_motor(const std::string& name_, size_t id_);
...
class swerve_talon : public swerve_motor {
   private:
	...
	controls::MotionMagicVoltage angle_voltage;
	controls::VelocityVoltage velocity_voltage;
   ...
   private:
	swerve_talon();
	swerve_talon(const std::string& name_, size_t id_);
	swerve_talon(const std::string& name_, size_t id_, bool is_drive_motor_);
	swerve_talon(const std::string& name_, size_t id_, std::shared_ptr<hardware::TalonFX> motor_, bool is_drive_motor_);

	auto factory_reset() -> void override;
	auto clear_faults() -> void override;
	auto configure_encoder(double conversion_factor) -> void override;
	auto configure_pidf(pidf_config config) -> void override;
